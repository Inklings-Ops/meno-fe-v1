...
class DynamicSliverAppBar extends StatefulWidget {
  const DynamicSliverAppBar({
    this.flexibleSpace = const SizedBox(height: kToolbarHeight),
    super.key,
    this.leading,
...
class _DynamicSliverAppBarState extends State<DynamicSliverAppBar> {
  ...
  // As long as the height is 0 instead of the sliver app bar a sliver to box
  // adapter will be used to calculate dynamically the size for the sliver
  // app bar
  double _height = 0;
  ...
  void _updateHeight() {
    ...
    // Needs to be called after the last frame has been rebuild otherwise
  ...
  Widget build(BuildContext context) {
    // Needed to lay out the flexibleSpace the first time, so we can calculate
        ...
        child: Stack(
          children: [
            Padding(
              ...
              child: Container(key: _childKey, child: widget.flexibleSpace),
            ),
            Positioned.fill(
              // 10 is the magic number which the app bar is pushed down
              // within the sliver app bar. Couldn't find exactly where this
              // number comes from and found it through trial and error.
              top: 10,
              child: Align(
                alignment: .topCenter,
                child: AppBar(
