...
class BibleRemoteDataSource with MLogger {
  BibleRemoteDataSource({required ApiClient api}) : _api = api;
  ...
  Future<List<VerseDto>?> downloadTranslation({
    ...
    try {
      log.d('BibleRemoteDataSource: downloading translation=$translation');

      // We bypass ApiClient's _executeRequest intentionally:
      // The Bible API returns raw binary (not a MenoResponse JSON wrapper),
      // and we need onReceiveProgress — neither fits ApiClient's contract.
      final response = await _api.raw.get<Uint8List>(
        '/api/default/',
      ...
      log.d('BibleRemoteDataSource: parsing ${bytes.length} bytes off-thread');
      ...
      log.d('BibleRemoteDataSource: parsed ${verses.length} verses');
      return verses;
    } on DioException catch (e) {
      if (CancelToken.isCancel(e)) {
        log.d('BibleRemoteDataSource: download cancelled');
        return null; // Cancellation is not an error — let caller decide.
      }
      log.e('BibleRemoteDataSource: download failed', error: e);
      throw MenoException(e.message ?? 'Failed to download Bible translation');
    } catch (e) {
      log.e('BibleRemoteDataSource: parse failed', error: e);
  ...
  Future<List<VerseDto>> parseAsset({
    ...
    log.d('BibleRemoteDataSource: parsing asset for translation=$translation');
    ...
    try {
      ...
      log.e('BibleRemoteDataSource: asset parse failed', error: e);
  ...
  Future<List<TranslationDto>> fetchAvailableTranslations() async {
    final results = await _api.getList<TranslationDto>(
      '/api/translations/',
      fromJson: (json) {
