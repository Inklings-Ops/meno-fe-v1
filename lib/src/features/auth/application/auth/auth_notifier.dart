...
class AuthNotifier extends ChangeNotifier {
  ...
  AuthNotifier(this._facade, this._onboardingLocal);
  ...
  Future<void> checkAuthenticated() async {
    ...
    _isOnboarded = (results[3] as bool);

    final isPartiallyAuthenticated = (_token == null) && (_user != null);
    final isAuthenticated = (_token != null) && (_user != null);

    if (!_isOnboarded) {
      _status = AuthStatus.notOnboarded;
      notifyListeners();
    } else if (isAuthenticated && _isOnboarded) {
      _status = AuthStatus.authenticated;
      notifyListeners();
    } else if (isPartiallyAuthenticated && _isOnboarded) {
  ...
  Future<void> completeOnboarding() async {
    ...
    _isOnboarded = true;
    return await _onboardingLocal.onboardingCompleted();
  }
}
