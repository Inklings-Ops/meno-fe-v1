...
class FolderListEvent with _$FolderListEvent {
  ...
  const factory FolderListEvent.addToFolder(
    Note note,
    Folder folder,
  ) = _AddToFolder;
  const factory FolderListEvent.updateList(
    Folder newFolder,
  ) = _UpdateList;
  const factory FolderListEvent.getFolderAndUpdateList(
    String id,
  ) = _GetFolderAndUpdateList;
