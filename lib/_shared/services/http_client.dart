...
/// - Integrated with ResponseDto wrapper
///
class HttpClient {
  HttpClient({
  ...
  Future<T> _executeRequest<T>(
    ...
    while (true) {
      try {
        ...
        final menoRes = ResponseDto.fromJson(response.data, (json) => json);
  ...
  MenoException _handleDioError(DioException e) {
    switch (e.type) {
      ...
      case DioExceptionType.badResponse:
        // Try to parse ResponseDto from error response
        if (e.response?.data is Map<String, dynamic>) {
          try {
            final errorResponse = ResponseDto.handleError(e.response!.data);
